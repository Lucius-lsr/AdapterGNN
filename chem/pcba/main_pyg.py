...
def eval(model, device, loader, evaluator):
    ...
    print(np.where(np.isnan(y_true)))
...
def main():
    ...
    model_param_group = []
    model_param_group.append({"params": model.gnn_node.prompts.parameters(), "lr": 0.001})
    model_param_group.append({"params": model.gnn_node.gating_parameter, "lr": 0.001})
    for name, p in model.gnn_node.named_parameters():
