...
class GNN_mask(torch.nn.Module):
    ...
    def __init__(self, num_clusters, gnn):
        ...
        self.label_embedding = torch.nn.Embedding(2 * num_clusters, num_atom_type)

    def init_label_emb(self, init):
        with torch.no_grad():
            self.label_embedding.weight = torch.nn.Parameter(init)

    def get_label_emb(self):
        return self.label_embedding.weight

    def from_pretrained(self, gnn_model_file):
        self.gnn.load_state_dict(torch.load(gnn_model_file))

    #def forward(self, x, edge_index, edge_attr):
    def forward(self, x, y, edge_index, edge_attr, device):
        p = self.gnn(x, edge_index, edge_attr)
