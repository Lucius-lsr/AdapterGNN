...
l3, = plt.plot(x, y3, '--o', ms=10, label='Full Fine-tune', linewidth=4, color='#1f77b4')
l4, = plt.plot(x, y4, '--o', ms=10, label='AdapterGNN', linewidth=4, color='#ff7f0e')

plt.xlabel(x_label, fontsize=30)
plt.xticks(x, fontsize=fontsize)
plt.yticks(fontsize=fontsize)
plt.ylabel('ROC-AUC(%)', fontsize=30)
first_legend = plt.legend(fontsize=30, loc="lower left", bbox_to_anchor=(0.2, 0), handles=[l1, l2], title='Average', title_fontsize=30)
ax = plt.gca().add_artist(first_legend)
plt.legend(fontsize=30, loc="lower right", handles=[l3, l4], title='BBBP', title_fontsize=30)
